...
<div x-data="aiChatWidget()" class="fixed bottom-6 right-6 z-50">
    <div x-show="openChat"
         ...
         class="bg-white rounded-xl shadow-xl border border-slate-200 w-80 sm:w-96 overflow-hidden mb-4 flex flex-col h-[480px]">

        <div class="bg-slate-950 px-4 py-3.5 text-white flex justify-between items-center shrink-0 border-b border-slate-800">
            <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-lg bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 flex items-center justify-center text-sm shadow-xs">
        ...
        <div x-ref="chatBox" class="p-4 bg-slate-50/70 flex-1 overflow-y-auto flex flex-col gap-3 text-xs">
            ...
            <template x-for="(msg, index) in messages" :key="index">
                <div class="flex gap-2 items-start" :class="msg.role === 'user' ? 'flex-row-reverse' : ''">
                    <template x-if="msg.role === 'ai'">
                        <div class="w-7 h-7 rounded-md bg-cyan-50 border border-cyan-200 text-cyan-700 shrink-0 flex items-center justify-center text-xs mt-0.5 shadow-2xs">
                            <i class="fa-solid fa-robot"></i>
                        </div>
                    </template>
                    <div class="p-3 shadow-xs max-w-[82%] text-xs leading-relaxed"
                         :class="msg.role === 'user' ? 'bg-cyan-700 text-white rounded-xl rounded-tr-xs' : 'bg-white border border-slate-200/80 text-slate-800 rounded-xl rounded-tl-xs'"
                         x-html="msg.text">
                    </div>
                </div>
            </template>
            ...
            <div x-show="isLoading" class="flex gap-2 items-start">
                <div class="w-7 h-7 rounded-md bg-cyan-50 border border-cyan-200 text-cyan-700 shrink-0 flex items-center justify-center text-xs mt-0.5">
                    <i class="fa-solid fa-robot"></i>
                </div>
                <div class="bg-white border border-slate-200 text-slate-500 text-xs p-3 rounded-xl rounded-tl-xs shadow-xs flex items-center gap-1.5">
        ...
        <div class="p-3 bg-white shrink-0">
            <form @submit.prevent="sendMessage()" class="flex gap-2">
                <input x-model="inputText" type="text" placeholder="Ketik pertanyaan ke AI..."
                       class="w-full bg-slate-50 border border-slate-200 rounded-md px-3.5 py-2 text-xs focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-600 focus:bg-white transition-all outline-none"
                       :disabled="isLoading">
                <button type="submit"
                        class="w-9 h-9 rounded-md bg-cyan-700 hover:bg-cyan-800 text-white shrink-0 flex items-center justify-center transition-colors shadow-xs disabled:opacity-50"
                        :disabled="isLoading">
    ...
    <button @click="openChat = !openChat; if(openChat) scrollToBottom()"
            class="w-12 h-12 p-3 bg-slate-950 hover:bg-slate-900 text-white rounded-full shadow-lg border border-slate-700 flex items-center justify-center transition-transform hover:scale-105 active:scale-95 focus:outline-none relative group"
            title="Tanya Asisten AI">
        <i class="fa-solid fa-sparkles text-cyan-400 text-base group-hover:rotate-12 transition-transform"></i>
        <span class="absolute -top-1 -right-1 w-3 h-3 bg-cyan-400 rounded-full border-2 border-white animate-pulse"></span>
    </button>
</div>
