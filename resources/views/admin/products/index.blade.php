<x-admin-layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl p-4.5 border border-slate-200/90 shadow-xs flex items-center gap-3.5 hover:border-slate-300 transition-colors">
            ...
            <div class="min-w-0">
                <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider truncate">Total Produk</p>
                <h4 class="text-xl font-bold text-slate-900 mt-0.5 truncate">{{ number_format($totalProducts ?? 0, 0, ',', '.') }} Produk</h4>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-4.5 border border-slate-200/90 shadow-xs flex items-center gap-3.5 hover:border-slate-300 transition-colors">
            ...
            <div class="min-w-0">
                <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider truncate">Produk Aktif (Tayang)</p>
                <h4 class="text-xl font-bold text-slate-900 mt-0.5 truncate">{{ number_format($activeProducts ?? 0, 0, ',', '.') }} Produk</h4>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-4.5 border border-slate-200/90 shadow-xs flex items-center gap-3.5 hover:border-slate-300 transition-colors">
            ...
            <div class="min-w-0">
                <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider truncate">Produk Di-Takedown</p>
                <h4 class="text-xl font-bold text-slate-900 mt-0.5 truncate">{{ number_format($takedownProducts ?? 0, 0, ',', '.') }} Produk</h4>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-xs border border-slate-200/90 overflow-hidden">
        ...
        <!-- SEARCH & FILTER BAR -->
        <div class="p-4 sm:p-5 border-b border-slate-100 flex flex-col lg:flex-row justify-between lg:items-center gap-3 bg-slate-50/50">
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 lg:pb-0">
                <a href="{{ auth()->user()->role === 'super_admin' ? route('super_admin.products.index', array_merge(request()->except(['status', 'page']))) : route('admin.products.index', array_merge(request()->except(['status', 'page']))) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors {{ empty($status) ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100' }}">
                ...
                <a href="{{ auth()->user()->role === 'super_admin' ? route('super_admin.products.index', array_merge(request()->except(['status', 'page']), ['status' => 'active'])) : route('admin.products.index', array_merge(request()->except(['status', 'page']), ['status' => 'active'])) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors {{ ($status ?? '') === 'active' ? 'bg-emerald-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100' }}">
                ...
                <a href="{{ auth()->user()->role === 'super_admin' ? route('super_admin.products.index', array_merge(request()->except(['status', 'page']), ['status' => 'takedown'])) : route('admin.products.index', array_merge(request()->except(['status', 'page']), ['status' => 'takedown'])) }}" 
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors {{ ($status ?? '') === 'takedown' ? 'bg-rose-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100' }}">
            ...
            <form action="{{ auth()->user()->role === 'super_admin' ? route('super_admin.products.index') : route('admin.products.index') }}" method="GET" class="flex items-center gap-2 w-full lg:w-auto">
                ...
                <div class="relative flex-1 lg:w-72">
                    <input type="text" name="search" value="{{ $search }}" class="w-full h-9 pl-9 pr-3 text-xs rounded-lg border border-slate-200 bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors placeholder:text-slate-400" placeholder="Cari nama produk atau toko...">
                ...
                <button type="submit" class="h-9 px-3.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs transition-colors shadow-xs cursor-pointer">Cari</button>
                @if($search || $status)
                    <a href="{{ auth()->user()->role === 'super_admin' ? route('super_admin.products.index') : route('admin.products.index') }}" class="h-9 px-3 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-colors border border-slate-200">Reset</a>
                @endif
            </form>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/70 transition-colors {{ !$product->is_active ? 'bg-rose-50/30' : '' }}">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="font-semibold text-slate-900 truncate max-w-[200px]">{{ $product->name }}</p>
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-slate-800 font-medium truncate max-w-[140px]">{{ $product->store->name ?? 'Toko' }}</span>
                            </div>
                        </td>
                        <td class="px-5 py-4">
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-14 text-center text-slate-400">
                            ...
                            <span class="text-sm font-semibold text-slate-700 block">Tidak Ada Produk Ditemukan</span>
