<x-admin-layout>
    <x-slot name="title">
        Edit Gudang Hub NDX - {{ config('app.name', 'NitipDong') }}
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Gudang Hub #{{ $warehouse->code }}</h1>
                <p class="text-xs text-slate-500 mt-0.5">Perbarui data lokasi dan PIC pusat distribusi NitipDongExpress.</p>
            </div>
        </div>

        <div class="bg-white rounded-2xl border border-slate-200/90 p-6 sm:p-8 shadow-xs">
            <form action="{{ route($prefix . '.warehouses.update', $warehouse) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="city" value="{{ old('city', $warehouse->city) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="province" value="{{ old('province', $warehouse->province) }}" required
                ...
                <div>
                    ...
                    <textarea name="address" rows="3" required
                              ...
                              class="w-full py-2.5 px-3.5 rounded-xl border border-slate-300 text-xs focus:border-cyan-600 focus:ring-1 focus:ring-cyan-500">{{ old('address', $warehouse->address) }}</textarea>
                    @error('address')
                        <p class="text-[11px] text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="any" name="lat" value="{{ old('lat', $warehouse->lat) }}" required
                               placeholder="-6.2088"
                               class="w-full py-2.5 px-3.5 rounded-xl border border-slate-300 font-mono text-xs focus:border-cyan-600 focus:ring-1 focus:ring-cyan-500">
                        @error('lat')
                            <p class="text-[11px] text-rose-600 mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" step="any" name="lng" value="{{ old('lng', $warehouse->lng) }}" required
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        <span class="text-xs font-semibold text-slate-700">Gudang hub ini beroperasi aktif</span>
                    </label>
                </div>
