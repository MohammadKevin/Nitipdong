    ...
    <div class="max-w-2xl bg-white rounded-lg shadow-xs border border-slate-200/90 overflow-hidden" x-data="{ 
        ...
        <form action="{{ auth()->user()->role === 'super_admin' ? route('super_admin.categories.store') : route('admin.categories.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                <a href="{{ auth()->user()->role === 'super_admin' ? route('super_admin.categories.index') : route('admin.categories.index') }}" class="h-8.5 px-3.5 flex items-center justify-center rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs font-semibold">
                    Batal
                </a>
