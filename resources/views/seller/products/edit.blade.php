<x-seller-layout>
    ...
    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-2">
        <div>
            ...
            <p class="text-xs text-[#8A93A6] mt-0.5">Perbarui rincian, harga, stok, atau status keaktifan produk Anda.</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-[#EFEBDF] overflow-hidden">
        <form action="{{ route('seller.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="price" name="price" x-model.number="price" value="{{ old('price', $product->price) }}" required min="0"
                                    placeholder="150000"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5" required
                            ...
                            class="w-full px-4 py-3 rounded-xl border border-[#E7E3D8] text-sm text-[#14213D] focus:ring-2 focus:ring-[#12A57F]/20 focus:border-[#12A57F] transition-all">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <div x-data="{ preview: null }">
                    <p class="text-xs text-slate-400 mb-3">Format didukung: JPG, JPEG, PNG, WEBP (Maks. 2MB). Pilih foto baru untuk mengganti foto lama.</p>
